...
const Header = () => {
    ...
    const {color,toggleColor,showColor,toggleTheme,theme} = usePortfolioContext();
  ...
  return (
    <header className=' z-[2] bg-bgColor sticky top-0' ref={headerRef}>
        <nav className='flex justify-between items-center py-6  w-[90%] mx-auto relative'>
            {/* logo */}
            ...
            <section className='flex gap-4'>
                ...
                {/* theme */}
                <figure className={` shadow bg-bgColor  grid items-center rounded-md`} onClick={toggleTheme}>
                    <button className='border border-gray-500 px-2 p-2 rounded-md'>
                        {theme ? <BsSunFill className="w-6 h-6"/> : <BsMoonFill className="w-6 h-6"/>}
                    </button>
                </figure>
                {/* color picker */}
                <aside className={` shadow bg-bgColor grid items-center rounded-md relative`}>
                    <button className='border border-gray-500 p-2 rounded-md' onClick={()=>toggleColor()}>
                        <div className='h-6 w-6 rounded-full'  style={{backgroundImage: `linear-gradient(to bottom, ${colors[color].color}, ${colors[color].gradient})`}}></div>
                    </button>
                    <div className={`absolute top-[100%] left-1/2 -translate-x-1/2 rounded-md shadow grid gap-2 mt-3 bg-bgColor  transition-all duration-500 px-2 overflow-hidden h-0 ${showColor ? "h-[11rem] py-2" : ""}`}> 
                        {colors.map((c,index)=>{
                            return (
                                <button key={index} style={{backgroundImage: `linear-gradient(to bottom, ${c.color}, ${c.gradient})`}} className={`h-6 w-6 rounded-full`} onClick={()=>toggleColor(index)}></button>
                            )
                        })}
                    </div>
                </aside>
